...
const tuneSchema = new mongoose.Schema(
  ...
  },
  {
    ...
    strict: 'throw',
    // Idem pour les descripteurs de requête (consultation de la base) sur les
    // modèles qui utilisent ce schéma.
    strictQuery: true,
    // Ajoute et maintient automatiquement des champs d’horodatage `createdAt`
    // et `updatedAt`.
    timestamps: true,
    // Par défaut, l’option `strict` ne s’applique pas aux documents imbriqués
    // (ex. ici les votes) : on l’exige donc à part.
...
async function search({
  ...
  // nouvelle `Query` basée sur la même requête en cours MongoDB, obtenue via
  // `getQuery()` sur la première.
