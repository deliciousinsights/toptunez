    ...
    it('should order recent-first by default', () => {
      return (
        request(app)
          ...
          .then((res) => {
            // On passe ici au `expect()` fourni par Chai, et à ses
            // [matchers](https://www.chaijs.com/api/bdd/#method_language-chains).
            // Malgré les combos possibles des chaînons `deep`, `include`,
            // `members` et consorts, il n'est pas possible de réaliser une
            // comparaison partielle traversant à la fois un tableau et les objets
            // qu'il contient, on doit donc préfiltrer les valeurs qui nous
            // intéressent dans ces derniers.
    ...
    it('should provide earlier links when beyond page 1', () => {
      ...
      return request(app)
        ...
        .then((res) => {
          ...
          // Chai `to.deep.equal()`.  On ne pouvait pas utiliser le
          // `.expect(obj)` de Supertest, car on ne vérifie ici qu’une partie du
          // corps de réponse.
