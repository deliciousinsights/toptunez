...
async function createTune(req, res, next) {
  try {
    ...
    res.send(201)
    // Toujours bien penser à appeler `next()`, ce n’est pas automatique comme
    // ce serait le cas dans Express suite à un `send()`…
    next()
  } catch (err) {
    next(err)
  }
...
async function listTunes(req, res, next) {
  try {
    ...
    // `req.version()` est présent en raison du `conditionalHandler` qui entoure
    // la configuration du gestionnaire.  Idée de la démo : seule une demande de
    // version explicitement 1.2+ permet la prise en compte du filtre `sortBy`.
    if (req.version() >= '1.2') {
...
async function voteOnTune(req, res, next) {
  try {
    ...
    // donc pas amené dans le `catch`. Mais pour nous, ça rend la requête
    // impossible à traiter (code HTTP 422).
    if (!tune) {
      ...
      // Bien penser au `return` ici pour court-circuiter la suite !
      return next(err)
    }
